...
- ♻️ 优化：偏好设置界面重设计——两列表单布局（左标签 100px / 右控件），高度从 710 → 450px，去除所有 section 副标题描述文字
- 🐛 修复：查询地址字段无法显示完整 URL（原宽度 184px 不足，现独占 CTRL_W=272px 整行）
- 🐛 修复：恢复默认按钮宽度/字体（80px/12pt）与录制按钮对齐
- 🐛 修复：选择文件夹面板默认定位到当前已设置的同步路径（setDirectoryURL_）
- 🐛 修复：VERB 标签与第一条释义间距过小（POS badge after 8→14pt，第一条 def before 0→4pt）
